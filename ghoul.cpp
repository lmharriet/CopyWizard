...
void ghoul::update()
{
    ...
    if (isFindWayOn) //±æÃ£±â on
    {
        ...
        if (astar->getFirstTile() && !isATK  && !isHit && !isIceState && !isDie) // °ÉÀ» ¶§
        {
        ...
        else if ( !isHit && !isIceState && !isDie) // °ÈÁö ¾Ê°í ÀÖÀ» ¶§
        {
...
void ghoul::stateATK()
{
    ...
    if (atkDirection[MONSTER_UP] && playerRC.left > rc.left && playerRC.right < rc.right + 40)
    {
        if (delay == 0 && (frameIndexL[STATEIMAGE::ATK].x == 3 || frameIndexR[STATEIMAGE::ATK].x ==2))
        {
            ...
            SOUNDMANAGER->play(str, false, soundVolum); // »ç¿îµå¼öÁ¤¿¹Á¤
            bulletDirection[MONSTER_DOWN] = false;
...
void ghoul::stateDIE()
{
    if (atkDirection[MONSTER_LEFT])
    {
        ...
        if (count % 5 == 0)
        {
            ...
            if (frameIndexL[STATEIMAGE::DIE].x < 3 && frameIndexL[STATEIMAGE::DIE].y == 4) //»ç¸Á ÀÎµ¦½º 6
            {
    ...
    else
    {
        ...
        if (count % 5 == 0)
        {
            ...
            if (frameIndexR[STATEIMAGE::DIE].x > 2 && frameIndexR[STATEIMAGE::DIE].y == 4) //»ç¸Á ÀÎµ¦½º 6
            {
